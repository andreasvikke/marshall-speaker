...
class MarshallAPIManager {
    ...
    async get_volume() {
        return await this.handle_int("netRemote.sys.audio.volume") / this.fixedMaxVolume;
    }
    async set_volume(value) {
        return await this.handle_set("netRemote.sys.audio.volume", value * this.fixedMaxVolume);
    }
